...
from . import swim
from .cardkit import (ACCENT_SWIM as ACCENT, BAD, BG, GOOD, HIST, LINE, MINT, MUTED, PANEL, TXT, TXT2, WARN, ZONE_EASY, ZONE_HARD, ZONE_MID,
                      attendance, header, panel as _panel, rounded as _rounded, setup_font as _setup_font)
...
def render_card(summary: dict) -> bytes:
    ...
    gs = fig.add_gridspec(6, 2, height_ratios=[0.42, 0.75, 0.55, 1.3, 1.5, 1.35], hspace=0.68, wspace=0.26,
    ...
    header(fig, hd, f"{u['name']}님의 수영 브리핑", f"{d.year}년 {d.month}월 {d.day}일 {summary['weekday']}요일", ready,
           ACCENT, "swim", "Y R C   S W I M   B R I E F")

    # 출석 달력
    at = fig.add_subplot(gs[1, :])
    active = {x["date"] for x in (summary.get("recent_sessions") or [])}
    attendance(fig, at, d, active, "swim", ACCENT)
    ...
    _panel(ax, "주간 수영 거리", "최근 8주 · m · 진한 막대 = 이번 주", icon="chart", fig=fig)
    ...
    ax.axis("off")
    _panel(ax, "최근 수영", "최근 14일 · 막대 = 거리 · 100m 페이스 · 심박 · 25m당 스트로크 · 칼로리", icon="swim", fig=fig)
    ...
    ax.axis("off")
    _panel(ax, "스트로크 효율 (핵심 과제)", "25m 한 번 갈 때 스트로크 수 · 적을수록 효율적", icon="muscle", fig=fig)
    if ef.get("spl"):
    ...
    ax.axis("off")
    _panel(ax, "오늘 식단", icon="salad", fig=fig, sub=f"목표 {diet['target_kcal']:,}kcal · 단백질 {diet['protein_g']}g · 탄수 {diet['carbs_g']}g · 지방 {diet['fat_g']}g" + ("" if diet["personalized"] else " (기본값)"))
    for i, (k, val) in enumerate(diet["meals"].items()):
        ...
        import textwrap
        lines = textwrap.wrap(val, 34)[:2]
        for j, ln in enumerate(lines):
            ax.text(0.07, y - 0.045 - j * 0.075, ln, color=TXT, fontsize=9 if len(lines) > 1 else 9.5, transform=ax.transAxes, va="center")
