...
## C-1. 실제로 쓰는 단축어 (애플워치 + 무료, 동작 5개)

iOS 단축어는 "운동" 기록을 못 꺼내지만, 애플워치가 러닝 중에만 기록하는 **달리기 속도**와 **심박 샘플**로 서버가 러닝을 복원합니다 (건강앱 내보내기 실측과 거리·시간 5% 이내, 평균심박 1~5 오차).

| # | 동작 | 설정 |
|---|---|---|
| 1 | 건강 샘플 찾기 | 유형 달리기 속도 · 시작일 과거 56일 · 단위 km/h · 그룹화 **시간** · "파일이 유실됨" 끄기 · 제한 끄기 |
| 2 | 건강 샘플 찾기 | 유형 걷기+달리기 거리 · 과거 56일 · km · 그룹화 **시간** · 필터 추가 **소스 이름 다음을 포함 "Watch"** (아이폰 중복 제거) |
| 3 | 건강 샘플 찾기 | 유형 심박수 · 과거 **14일** · 그룹화 **없음** (원본 샘플) |
| 3b | 건강 샘플 찾기 | 유형 달리기 보폭 길이 · 과거 56일 · 단위 m · 그룹화 **시간** · 필터 소스 이름 포함 "Watch" 필수 (케이던스 = 속도 ÷ 보폭, 실측 ±3) |
| 4 | 텍스트 | 아래 형식. 각 줄 아래에 변수 토큰 (dates 는 토큰 → 시작일 → 날짜 포맷 ISO 8601, values 는 건강 샘플 그대로) |
| 5 | URL 콘텐츠 가져오기 | `https://<서버>/ingest/<token>` · POST · 본문 요청 **파일** = 4번 텍스트 |
...
주의: 찾기 동작을 앞 동작 바로 아래에 추가하면 자동으로 "필터링" 으로 바뀝니다 → 파란 건강 샘플 토큰 → **변수 지우기** 로 되돌리기. 설정 → 앱 → 단축어 → 고급 → **대량의 데이터 공유 허용** 을 켜야 합니다. 자동화: 시간대 매일 06:40, 즉시 실행.

서버가 받은 원본은 `GET /ingest/<token>/last` 로 확인할 수 있습니다.

## C-2. (참고) 반복문 방식 단축어 — 느려서 권장하지 않음
